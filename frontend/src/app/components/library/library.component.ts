...
export class LibraryComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  // Map from videoId to processing job (for displaying progress in library)
  videoToJobMap = new Map<string, VideoProcessingJob>();
  ...
  private hasAutoCollapsedOnce = false; // Track if auto-collapse has run to prevent resetting user's manual expand/collapse
  ...
  // Children config for cascade - accordion disabled but progress bars enabled
  listChildrenConfig: ChildrenConfig = {
    enabled: false, // No accordion arrows
    expandable: false, // Not expandable
    defaultExpanded: false,
    showMasterProgress: true, // YES show progress bars on items
    generator: (item: any) => [], // No children
    masterProgressCalculator: (item: any) => this.calculateVideoMasterProgress(item) ?? 0
  };
  ...
  async ngOnInit() {
    ...
    // Subscribe to video processing queue jobs to show progress in library
    this.subscriptions.push(this.videoProcessingQueueService.getJobs().subscribe(jobsMap => {
      console.log('[LibraryComponent] Processing queue jobs updated:', jobsMap.size);

      // Update video-to-job mapping
      this.videoToJobMap.clear();
      jobsMap.forEach((job) => {
        if (job.videoId) {
          this.videoToJobMap.set(job.videoId, job);
          console.log(`[LibraryComponent] Mapped video ${job.videoId} to job ${job.id} (progress: ${job.overallProgress}%)`);
        }
      });

      // Trigger change detection to update progress bars
    ...
    // Subscribe to suggestion accepted events from WebSocket
    this.subscriptions.push(this.socketService.onSuggestionAccepted().subscribe(event => {
      console.log('[LibraryComponent] Suggestion accepted event received:', event);

      // Find the video and clear its suggestion
      const video = this.videos.find(v => v.id === event.videoId);
      if (video) {
        video.suggested_title = null;
        video.filename = event.newFilename;

        // Also update in filteredVideos
        const filteredVideo = this.filteredVideos.find(v => v.id === event.videoId);
        if (filteredVideo) {
          filteredVideo.suggested_title = null;
          filteredVideo.filename = event.newFilename;
        }
        ...
        // Also update DOM to clean up styled suggestion text
        this.updateListItemSecondaryText(video);
      }
    }));

    // Subscribe to suggestion rejected events from WebSocket
    this.subscriptions.push(this.socketService.onSuggestionRejected().subscribe(event => {
      console.log('[LibraryComponent] Suggestion rejected event received:', event);
   ...
   * Prioritizes "New" videos (last 24 hours) for immediate display
   */
  async loadVideos() {
    try {
      ...
      if (this.isInitialLoad) {
        ...
        // Load ALL videos initially (they're sorted by date DESC on backend)
        // This ensures we get all "New" videos (last 24h) immediately
        const initialResponse = await this.databaseLibraryService.getVideos(200, 0);

        // Separate new videos (last 24 hours) from older videos
        const now = new Date();
        const newVideos: DatabaseVideo[] = [];
        const olderVideos: DatabaseVideo[] = [];

        initialResponse.videos.forEach(video => {
          const downloadDate = this.parseDateSafely(video.download_date || video.added_at);
          const hoursSinceDownload = (now.getTime() - downloadDate.getTime()) / (1000 * 60 * 60);
        ...
        // Show new videos first, then append a few older ones
        this.videos = [...newVideos, ...olderVideos.slice(0, Math.max(0, 50 - newVideos.length))];

        console.log(`[loadVideos] Loaded ${newVideos.length} new videos (24h) + ${this.videos.length - newVideos.length} older videos`);
        ...
        // On subsequent loads, use cache for fast loading (cache is cleared on video modifications)
        console.log(`[loadVideos] BEFORE fetch: this.videos.length = ${this.videos.length}`);
        const response = await this.databaseLibraryService.getVideos(999999, 0, true); // Use cache!
        console.log(`[loadVideos] Fetched ${response.videos.length} videos from ${response.videos === this.videos ? 'cache' : 'API'}`);
  ...
  private async loadRemainingVideos(totalCount: number) {
    ...
    try {
      ...
      const batchSize = 100;
      let offset = 200; // Start after the initial 200
  ...
  async acceptVideoSuggestion(video: DatabaseVideo, customFilename?: string) {
    try {
      ...
      if (result?.success) {
        ...
        // Trigger reactive update of the cascade list
        this.updateVideosAsListItems();
        this.cdr.detectChanges();

        // Also update DOM directly to ensure styled suggestion text is removed immediately
        this.updateListItemSecondaryText(video);
  ...
  onListContextMenu(data: { event: MouseEvent; item: DatabaseVideo }) {
    ...
    // If this is a ghost (reference to parent/child in different group),
    // find the real video object
    const isGhost = (data.item as any)['isGhostParent'] || (data.item as any)['isGhostChild'];
    const realVideo = isGhost
      ? this.filteredVideos.find(v => v.id === data.item.id) || data.item
      : data.item;

    // Finder-like behavior: If right-clicking on an unselected item, clear selection and select only that item
    if (!this.selectedVideos.has(realVideo.id)) {
      // Clear all selections
      this.selectedVideos.clear();
      // Select only the right-clicked item
      this.selectedVideos.add(realVideo.id);
      // Update cached count
      this.updateSelectedCount();
      // Highlight the real video (not the ghost)
      this.highlightedVideo = realVideo;
      this.updateAllSelectedState();
    }
    // If right-clicking on a selected item, keep the current selection (allows multi-item operations)

    // Set context menu position and video (use real video, not ghost)
    this.contextMenuPosition = {
      x: data.event.clientX,
      y: data.event.clientY
    };
    this.contextMenuVideo = realVideo;
        ...
        if (result && result.success && result.jobsToAdd) {
          setTimeout(() => {
            for (const jobData of result.jobsToAdd) {
              // REMOVED: this.analysisQueueService.addPendingJob(jobData); // TODO: Migrate to VideoProcessingQueueService
            }
  ...
  private async analyzeVideos(videos: DatabaseVideo[]) {
    ...
    await this.videoOperationsService.analyzeVideos(videos);
  ...
  private async processVideos(videos: DatabaseVideo[]) {
    for (const video of videos) {
      try {
        ...
        // REMOVED: addToPendingQueue call - TODO: Migrate to VideoProcessingQueueService
      } catch (error: any) {
  ...
  async openAnalyzeDialog(video: DatabaseVideo) {
    await this.videoOperationsService.analyzeVideo(video);
  }
  ...
  async downloadFromUrl() {
    await this.videoOperationsService.downloadFromUrl();
  }
   ...
   * Uses the new VideoProcessingQueueService for accurate real-time progress
   */
  private generateVideoProcessingStages(video: DatabaseVideo): CascadeChild[] {
    // First check new processing queue (hierarchical jobs)
    const job = this.videoToJobMap.get(video.id);

    // Only show processing stages for jobs that are still in progress
    // Completed/failed jobs should not show accordion arrows
    if (job && job.childProcesses.length > 0 && job.overallStatus !== 'completed' && job.overallStatus !== 'failed') {
      // Map child processes to CascadeChild format
      return job.childProcesses.map(child => {
        // Map ProcessStatus to CascadeChildStatus
        ...
        return {
          ...
          icon: getProcessIcon(child.type),
          status: status,
          progress: {
            value: child.progress,
            indeterminate: status === 'active' && child.progress < 5
          },
          error: child.error
        };
      });
    ...
    const currentStage = oldJob?.stage || state.stage;
    const progress = oldJob?.progress || state.progress;
   ...
   * Returns null when complete to hide progress bar
   */
  private calculateVideoMasterProgress(video: DatabaseVideo): number | null {
    // First check new processing queue (already has overall progress calculated)
    const job = this.videoToJobMap.get(video.id);
    if (job) {
      // Hide progress bar when job is complete or failed
      if (job.overallStatus === 'completed' || job.overallStatus === 'failed') {
        return null;
      }
      return job.overallProgress || 0;
    }

    // Fallback to calculating from children for old jobs
    const children = this.generateVideoProcessingStages(video);

    if (children.length === 0) return null;  // No progress to show
  ...
  private autoCollapseOldSections() {
    // Only run auto-collapse once on initial load to avoid overwriting user's manual expand/collapse
    if (this.hasAutoCollapsedOnce) {
      console.log('[LibraryComponent] Auto-collapse already ran, skipping to preserve user state');
      return;
    }
    ...
    setTimeout(() => {
      ...
      // Mark that auto-collapse has run to prevent it from running again
      this.hasAutoCollapsedOnce = true;
